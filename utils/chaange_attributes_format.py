...
out = pd.read_csv("results/Reelcraft/reelcraft_attributes_table.csv", sep=',')
...
with open('results/Reelcraft/reelcraft_attributes.csv', 'wb') as csvfile:
	...
	for item in catalog_numbers:
		if str(catalog_attributes[item]) != 'nan':
			...
			html = catalog_attributes[item].replace('<font>', '').replace('</forn>', '').replace('<b>', '').replace('</b>', '')
			content = Selector(text=html)
			for tr in content.xpath('//table/tr'):
				if custom_extractor(tr, './td[2]/text()') == ' ':
					continue
