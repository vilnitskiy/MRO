...
out = pd.read_csv("mro/spiders/csv_data/flexco/0695487001508855201_0.csv", sep=',')
...
'''
f = open('mro/spiders/csv_data/flexco/viewstage.txt', 'r')
viewstage = f.read()
f.close()
...
class Weg(scrapy.Spider):
    ...
    def start_requests(self):
        for code in catalog_item_codes:
            ordering_number = item_code_ordering_number[code]
            descr = re.findall(r'Flexco \d{5} (.+)', item_codes_description[code])[0]
            url = 'https://www.e-services.flexco.com/eCollaboration/ecatalog/aspx/ItemSpecification.aspx?OrderingNumber=' + ordering_number + '&ItemCode=' + code + '&ItemDescription=' + descr
            yield scrapy.Request(url=url, callback=self.parse_item2, meta={'proxy': '108.59.14.208:13041', 'code': code})
    ...
    def construct_table(self, table):
        table = re.sub(r'style=".+"', '', table)
        table = re.sub(r'cellpadding=".+"', '', table)
        table = re.sub(r'border=".+"', '', table)
        table = re.sub(r'id=".+"', '', table)
        table = re.sub(r'class=".+"', '', table)
        table = re.sub(r'align=".+"', '', table)
        table = re.sub(r'width=".+"', '', table)
        table = re.sub(r'valign=".+"', '', table)
    ...
    def parse_item2(self, response):
        ...
        E['item_code'] = code
        E['add_descr'] = self.construct_table(descr)
