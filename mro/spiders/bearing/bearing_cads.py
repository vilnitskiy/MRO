...
class BearSpider(CrawlSpider):
    ...
    pr = False
    data = pandas.read_csv("mro/spiders/csv_data/Bearingfinder/NTN.csv", sep=',')
    catalog = list(data.catalog_number)
    ids = list(data.id)
    catalog_ids = dict(zip(catalog, ids))

    proxy = 'http://108.59.14.208:13041/'

    urls = []
    for sku in catalog:
        if sku == 'ML71905HVDUJ84S':
            pr = True
        if pr:
            urls.append(
    ...
    def parse(self, response):
        ...
        if response.xpath('//nav[@id="plp-product-title"]').extract():
            ...
            url = 'http://bearingfinder.ntnamericas.com' + url

            response.request.cookies.update({
                'p.cad': '%7B%22systemUnit%22%3A%22inch%22%2C%22DL_3D%22%3A%22dxf3d%22%7D',
                'p.caduser_1': '%5B%7B%22V%22%3A%22Gosha%22%2C%22AID%22%3A106%2C%22MID%22%3A0%2C%22Value%22%3A%22Gosha%22%7D%2C%7B%22V%22%3A%22Gosha%22%2C%22AID%22%3A107%2C%22MID%22%3A0%2C%22Value%22%3A%22Gosha%22%7D%2C%7B%22V%22%3A%22gosha%40gosha.com%22%2C%22AID%22%3A130%2C%22MID%22%3A0%2C%22Value%22%3A%22gosha%40gosha.com%22%7D%2C%7B%22V%22%3A%22gosha%40gosha.com%22%2C%22AID%22%3A1745%2C%22MID%22%3A0%2C%22Value%22%3A%22gosha%40gosha.com%22%7D%2C%7B%22V%22%3A%22MRO%20%2F%20end%20user%22%2C%22AID%22%3A1792%2C%22MID%22%3A0%2C%22Value%22%3A%22MRO%20%2F%20end%20user%22%7D%2C%7B%22V%22%3A%22Free%22%2C%22AID%22%3A112%2C%22MID%22%3A0%2C%22Value%22%3A%22Free%22%7D%2C%7B%22V%22%3A%2201001%22%2C%22AID%22%3A122%2C%22MID%22%3A0%2C%22Value%22%3A%2201001%22%7D%2C%7B%22V%22%3A%22UNITED%20STATES%20OF%20AMERICA%22%2C%22AID%22%3A124%2C%22MID%22%3A0%2C%22Value%22%3A%22UNITED%20STATES%20OF%20AMERICA%22%7D%5D',
                # 'p.ins': 'True'
            })
    ...
    def get_url(self, response):
        if response.xpath('//a[text()="3D"]/@data-url'):
            url = response.xpath('//a[text()="3D"]/@data-url').extract()[0]
    ...
    def parse_cad_page(self, response):
        ...
        payload = '{"cadStoreID":' + cadstoreid + ',"downloadFormat":"dxf3d","insertSystem":"ALIBRE_DESIGN_3d","downloadDir":"c:\\\_download"}'
    ...
    def get_cad_direct_url(self, response):
        ...
        if 'CadDownloadUrl' in response.body:
            ...
            try:
                ...
                file_name = '%s.dxf' % urllib.quote_plus(str(catalog_number))
                with open('bear_3d_download/' + file_name, 'wb') as file:
