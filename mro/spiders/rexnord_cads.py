...
class RexnordCadCrawl(scrapy.Spider):
	...
	def parse_item(self, response):
		...
		req = urllib2.Request('https://www.rexnord.com/api/v1/products/' + productId + '?expand=documents,specifications,styledproducts,htmlcontent,attributes,crosssells,relatedproductbrand,excludeconfiguredproduct')
	...
	def download_cad(self, response):
		...
		if 'error' in data:
			...
			new_url = 'https://www.product-config.net/catalog3/cad?d=rexnord.cfg&id={}&f=dxf-default&p0=REV_DATE&v0=10-1-2017'.format(cadid)
			return scrapy.Request(
				url=new_url,
				callback=self.download_cad,
				meta={'catalog_number': catalog_number, 'cadid': cadid, 'proxy': proxy}
