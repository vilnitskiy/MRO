...
class Mcr(scrapy.Spider):
    ...
    def login(self,response):
        ...
        return [scrapy.Request(url=url, cookies=self.cookies, callback=self.login2, dont_filter=True, meta={'key': key})]
    ...
    def request(self, row, key):
        url = 'https://www.tracepartsonline.net/(S(' + key + '))/content.aspx?SKeywords=' + urllib.quote_plus(row) + '&SDomain=1&st=4&sa=0&Class=SKF&clsid=%2fF_SKF%2f&ttl=SKF'
    ...
    def parse_item3(self, response):
        ...
        url = 'https://www.tracepartsonline.net/(S('+key+'))/rpc/GetDownloadsStatus.aspx?CommandName=DirectDownloadingCommand&ClientType=0&queueId=' + data['results']
        return scrapy.Request(url=url, 
